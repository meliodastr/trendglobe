export async function fetchGitHubSignals(){
  // GitHub trending doesn't have an official API; this placeholder uses GitHub blog RSS.
  const url = 'https://github.blog/feed/';
  const items = await fetchRSS(url);
  return items.slice(0,40).map(x => ({ source:'github', term: guessTerm(x.title), title: x.title, url: x.link, weight: 0.4, region:'GLOBAL', category:'tech' }));
}
