export async function fetchYouTubeSignals(){
  // YouTube RSS for channels is possible, but "trending" needs API.
  // Here we use Google News RSS with site:youtube.com queries as a workaround.
  const url = 'https://news.google.com/rss/search?q=site:youtube.com+trending&hl=en-US&gl=US&ceid=US:en';
  const items = await fetchRSS(url);
  return items.map(x => ({ source:'youtube', term: guessTerm(x.title), title: x.title, url: x.link, weight: 0.5, region:'US', category:'culture' }));
}
